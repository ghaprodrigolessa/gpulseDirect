...
function AnamnesePsicologia() {
  ...
  console.log('ANAMNESE PSICOLOGIA!!!')
    ...
    return (
      ...
      >
        ...
        <div
          ...
          <div id="CORPO DO DOCUMENTO - ANAMNESE - CRP"
            ...
            {evolucaoTexto(176, 'DOENÇA DE BASE', 1, 'textarea', 2000, 600)}
            {evolucaoTexto(177, 'SÚMULA DE AVALIAÇÃO, EXAME PSÍQUICO E ESTADO AFETIVO GERAL', 1, 'textarea', 2000, 600)}
            {evolucaoSelecaoSimples(129, 'ESTADO CIVIL', 1)}
            {evolucaoSelecaoSimples(130, 'POSSUI FILHOS', 1)}
            {evolucaoSelecaoMultipla(178, 'PARTICIPAÇÃO DA FAMÍLIA', 1)}
            {evolucaoSelecaoSimples(126, 'RELIGIÃO', 1)}
            {evolucaoSelecaoSimples(135, 'ETILISTA', 1)}
            {evolucaoSelecaoSimples(134, 'TABAGISTA', 1)}
            {evolucaoSelecaoSimples(179, 'DROGAS ILÍCITAS', 1)}
            {evolucaoTexto(180, 'INFORMAÇÕES ADICIONAIS', 1, 'textarea', 2000, 600)}
            {evolucaoSelecaoMultipla(181, 'HISTÓRIA DE SUPORTE', 1)}
            {evolucaoTexto(182, 'HISTÓRIA DE SUPORTE - OUTRAS INFORMAÇÕES', 1, 'textarea', 2000, 600)}
            {evolucaoSelecaoSimples(183, 'ESTADO DE ALERTA', 1)}
            {evolucaoSelecaoSimples(184, 'COMUNICAÇÃO', 1)}
            {evolucaoSelecaoSimples(185, 'ATENÇÃO', 1)}
            {evolucaoSelecaoSimples(186, 'MEMÓRIA', 1)}
            {evolucaoSelecaoSimples(187, 'PENSAMENTO/DISCURSO', 1)}
            {evolucaoSelecaoSimples(188, 'ORIENTAÇÃO', 1)}
            {evolucaoSelecaoSimples(189, 'SENSO/PERCEPÇÃO', 1)}
            {evolucaoSelecaoSimples(190, 'ESTADO EMOCIONAL GERAL', 1)}
            {evolucaoSelecaoSimples(191, 'DECLÍNIO COGNITIVO APARENTE', 1)}
            {evolucaoSelecaoMultipla(192, 'DEMANDA DE ATENDIMENTO', 1)}
            {evolucaoSelecaoMultipla(193, 'JUSTIFICATIVA DO ATENDIMENTO', 1)}
            {evolucaoSelecaoSimples(194, 'FREQUÊNCIA DO ATENDIMENTO', 1)}
            {evolucaoSelecaoMultipla(195, 'JUST. PARA ATENDIMENTO SEMANAL', 1)}
            {evolucaoSelecaoMultipla(196, 'JUST. PARA MONITORAMENTO QUINZENAL', 1)}
            {evolucaoSelecaoMultipla(197, 'JUST. PARA MONITORAMENTO MENSAL', 1)}
            {evolucaoSelecaoMultipla(198, 'PROPOSTAS', 1)}
  ...
  function Print() {
    return (
      <div id="IMPRESSÃO - ANAMNESE - CRP" className="print">
  ...
  function printDiv() {
    ...
    axios.get('http://192.168.100.6:3333/pool_evolucoes_valores/' + idatendimento).then((response) => {
      ...
      setprintdocumento(1);
      setTimeout(() => {
        let printdocument = document.getElementById("IMPRESSÃO - ANAMNESE - CRP").innerHTML;
  ...
  return (
    <div style={{
      display: tipodocumento == 'ANAMNESE - CRP' && conselho == 'CRP' && statusdocumento != null ? 'flex' : 'none',
      flexDirection: 'column', justifyContent: 'center',
    }}>
      <Campos></Campos>
      <div>
