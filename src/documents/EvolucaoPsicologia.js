...
function EvolucaoPsicologia() {
  ...
  useEffect(() => {
    if (tipodocumento == 'EVOLUÇÃO ESTRUTURADA - CRP' && conselho == 'CRP') {
      axios.get('http://192.168.100.6:3333/pool_evolucoes_valores/').then((response) => {
        ...
        setregistros_antigos(x.filter(item => item.evolucao == 'EVOLUÇÃO ESTRUTURADA - CRP' && item.idevolucao < iddocumento));
      ...
      >
        ...
        <div
          ...
          <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <EvolucaoTexto idcampo={200} campo={'SÚMULA DO ACOLHIMENTO, EXAME PSÍQUICO E ESTADO AFETIVO GERAL'} obrigatorio={1} tipo={'textarea'} lenght={2000} width={500}></EvolucaoTexto>
            <EvolucaoSelecaoMultipla idcampo={201} campo={'FINALIZAÇÃO'} obrigatorio={1}></EvolucaoSelecaoMultipla>
            <EvolucaoTexto idcampo={202} campo={'OBSERVAÇÕES'} obrigatorio={1} tipo={'textarea'} lenght={2000} width={500}></EvolucaoTexto>
          </div>
  ...
  function printDiv() {
    setprintdocumento(1);
    setTimeout(() => {
      var divContents = document.getElementById("FORMULÁRIO - EVOLUÇÃO PSICO").innerHTML;
      var a = window.open();
